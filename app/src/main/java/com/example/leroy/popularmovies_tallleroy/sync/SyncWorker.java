...
public class SyncWorker {
    ...
    static final String RATING = "vote_avg.desc";
...
//    };
    ...
    public static void performSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult, Context context) {
        ...
        if (Utility.getPreferredSortOrder(context).equals(R.string.pref_entry_sort_order_rating)) {
            sortValue = RATING;
        }
//
...
//            e.printStackTrace();
    ...
    private static void processJSON(String strWhole) {
        try {
            JSONObject jsonWhole = new JSONObject(strWhole);
            JSONArray results = jsonWhole.getJSONArray("results");
            ArrayList<MovieSummary> movieSummaries = new ArrayList<MovieSummary>(results.length());
            for(int i=0; i < results.length(); i++) {
                MovieSummary ms = new MovieSummary(results.getJSONObject(i));
                movieSummaries.add(ms);
            }
            for (MovieSummary ms : movieSummaries) {
                if (ms.getId() == MovieSummary.INVALID_ID) {
                    Log.d(LOG_TAG, "invalid MovieSummary");
                }
            }
        } catch (JSONException e) {
            e.printStackTrace();
